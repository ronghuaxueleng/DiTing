...
export default function Detail() {
  ...
  const [refPanelTab, setRefPanelTabRaw] = useState<
    "segments" | "immersive" | "mindmap"
  >(() => {
    return (
      (localStorage.getItem("detail-ref-panel-tab") as
        | "segments"
        | "immersive"
        | "mindmap") || "mindmap"
    );
  });
  const setRefPanelTab = useCallback(
    (tab: "segments" | "immersive" | "mindmap") => {
  ...
  const [selectedVersion, setSelectedVersion] = useState<CacheEntry | null>(
    null,
  );
  ...
  const [collapsedPanel, setCollapsedPanel] = useState<"left" | "right" | null>(
    () => {
      return (
        (localStorage.getItem("detail-split-collapsed") as
          | "left"
          | "right"
          | null) || null
      );
  ...
  const [vertCollapsed, setVertCollapsed] = useState<"top" | "bottom" | null>(
    () => {
      return (
        (localStorage.getItem("detail-vert-collapsed") as
          | "top"
          | "bottom"
          | null) || null
      );
  ...
  const scrollToNoteHeadingRef = useRef<((headingText: string) => void) | null>(
    null,
  );
  ...
  const applyVertDragPct = useCallback(
    (rawPct: number) => {
  ...
  const handleVertDividerMouseDown = useCallback(
    (e: React.MouseEvent) => {
      e.preventDefault();
  ...
  const { data: video, isLoading: isVideoLoading } = useQuery({
    ...
    refetchInterval: (query) => {
      ...
      const isActive =
        v?.latest_status === "processing" || v?.latest_status === "pending";
  ...
  const [mobileLayout, setMobileLayout] = useState<"scroll" | "split">(() => {
    return (
      (localStorage.getItem("detail-mobile-layout") as "scroll" | "split") ||
      "split"
    );
  });
  ...
  const updatePolicyMutation = useMutation({
    mutationFn: (data: {
      policy: "keep_forever" | "custom" | null;
      expires: string | null;
    }) =>
      updateVideoCachePolicy(sourceId!, {
        cache_policy: data.policy,
        cache_expires_at: data.expires,
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["video", sourceId] });
  ...
  const handleAppendCache = async (quality: string) => {
    ...
    try {
      ...
      showToast(
        "error",
        t("detail.appendCache.failed") + ": " + (e.message || String(e)),
      );
    }
  };
  ...
  return (
    <div className="h-screen flex flex-col bg-[var(--color-bg)] overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            {highlightQuery && (
              <div className="hidden sm:flex items-center gap-2 text-sm bg-yellow-50 dark:bg-yellow-900/20 px-3 py-1.5 rounded-full border border-yellow-200 dark:border-yellow-800">
                ...
                <button
                  onClick={() =>
                    navigate(`/detail/${encodeURIComponent(sourceId!)}`, {
                      replace: true,
                    })
                  }
            ...
            <button
              ...
              title={
                mobileLayout === "split"
                  ? t("detail.layout.scroll")
                  : t("detail.layout.split")
              }
            >
              {mobileLayout === "split" ? (
          ...
          >
            ...
            {isDesktop ? (
                ...
                {vertCollapsed !== "bottom" && (
                  ...
                  >
                    {contentTab === "notes" || contentTab === "mindmap" ? (
                        ...
                        >
                          ...
                          <div className="flex bg-[var(--color-bg)] p-0.5 rounded-md">
                            ...
                            <button
                              ...
                              className={`px-2.5 py-0.5 text-xs font-medium rounded transition-all flex items-center gap-1 ${
                        ...
                        <div className="flex-1 min-h-0 overflow-y-auto">
                          {refPanelTab === "mindmap" ? (
                            <MindmapPanel
                              ...
                              onNodeClick={(text) =>
                                scrollToNoteHeadingRef.current?.(text)
                              }
                      ...
                      /* Manual Notes Mode in Bottom Split Container */
                      <div className="flex flex-col h-full overflow-hidden">
                        <div className="px-4 py-3 border-b border-[var(--color-border)] flex justify-between items-center bg-[var(--color-card-muted)] shrink-0">
                        ...
                        <div className="flex-1 p-4 overflow-y-auto bg-[var(--color-bg)]/50 min-h-0 custom-scrollbar">
                          {isEditingNotes ? (
                            <div className="space-y-3 h-full flex flex-col">
                              <textarea
                                ...
                                className="flex-1 w-full min-h-[150px] bg-[var(--color-bg)] border border-[var(--color-border)] rounded-lg p-3 text-sm focus:ring-1 focus:ring-[var(--color-primary)] outline-none resize-none font-mono leading-relaxed custom-scrollbar"
                            ...
                            <div className="prose prose-sm dark:prose-invert max-w-none text-[var(--color-text)] pb-4">
                              <ReactMarkdown
                                remarkPlugins={[remarkGfm, remarkMath]}
                                rehypePlugins={[rehypeKatex]}
                              >
                                {preprocessLaTeX(video.notes)}
                              </ReactMarkdown>
                            </div>
                          ) : (
                            <div
                              className="h-32 flex flex-col items-center justify-center text-[var(--color-text-muted)] cursor-pointer hover:bg-[var(--color-bg-muted)]/50 rounded-lg border border-dashed border-[var(--color-border)] transition-all duration-200 group mt-4"
                              onClick={() => {
                                setNotesContent("");
                                setIsEditingNotes(true);
                              }}
              ...
              <div className={mobileLayout === "split" ? "flex-1 min-h-0 overflow-y-auto" : "space-y-6"}>
                ...
                {contentTab !== "notes" && (
                  <div
                    className={`bg-[var(--color-card)] border border-[var(--color-border)] rounded-xl overflow-hidden shadow-sm transition-all duration-200 ${isEditingNotes ? "ring-2 ring-[var(--color-primary)]/20" : ""}`}
                  >
                    ...
                    <div className="p-4 overflow-y-auto bg-[var(--color-bg)]/50">
                      {isEditingNotes ? (
              ...
              >
                ...
                {contentTab === "segments" ? (
                  segments?.length === 0 ? (
                    ...
                    <div
                      className={`flex-1 space-y-4 pr-2 ${mobileLayout === "split" ? "overflow-y-auto" : "overflow-y-auto lg:overflow-y-auto"}`}
                    >
                      {segments?.map((segment) => (
                        <SegmentCard
                          key={segment.id}
                  ...
                  <div
                    className={`flex-1 lg:min-h-0 h-full ${mobileLayout === "split" ? "min-h-0 overflow-hidden" : "min-h-[60vh] lg:overflow-hidden"}`}
                  >
                    <ImmersiveView
                      segments={segments || []}
                      currentTime={currentTime}
                  ...
                  <div
                    className={`flex-1 overflow-hidden ${mobileLayout === "split" ? "min-h-0" : "lg:overflow-hidden"}`}
                  >
                    <MindmapPanel
                      noteContent={activeNote?.content ?? ""}
                      onSeek={(time) => {
                  ...
                  <div
                    className={`flex-1 overflow-hidden ${mobileLayout === "split" ? "" : "lg:overflow-hidden"}`}
                  >
                    <NoteView
                      sourceId={sourceId!}
                      segments={segments || []}
      ...
      <ConfirmModal
        ...
        message={t("detail.confirm.deleteCacheMessage", {
          title: video?.title || sourceId,
        })}
